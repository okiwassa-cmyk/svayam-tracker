...
以下の形式でJSONのみを返してください（他のテキストは不要）：
{
  ...
  "advice": "一言。まずこの皿の良いところ（揃っている味・組み合わせ）に触れる。足りない味の指摘は、簡単に足せる具体案があるときだけ、最後に軽く。"
}
...
- 塩：味噌・醤油・塩・岩塩・塩麹、海藻（わかめ・青さ/アオサ・もずく・ひじき・昆布）、煮干し出し、ナンプラー、佃煮、たくあん
- 辛：生姜、唐辛子・島唐辛子、胡椒、マスタード、和がらし、ニンニク、ネギ、大根（生）、クミン、トリカトゥー
- 苦：葉物野菜（小松菜・ほうれん草・つるむらさき・モロヘイヤ・空芯菜・青梗菜・モリンガ）、ゴーヤ、ターメリック、パクチー、フェヌグリーク
- 渋：豆類全般（大豆・小豆・金時豆・ムング豆・レンズ豆・ひよこ豆など）、豆腐・厚揚げ・納豆・豆乳、ブロッコリー、オクラ、キャベツ、ごぼう、きのこ類、こんにゃく、切り干し大根、レンコン、茄子
一皿に複数の味が該当することが普通です。該当するものはすべて挙げてください（漏らさない）。
1つの食材が複数の味を持つことがあります。特に**豆・豆腐・納豆は「甘」と「渋」の両方**なので、必ず両方を挙げてください（片方だけにしない）。
カファ・ピッタ体質にとっては苦味・渋味が理想、甘味・酸味・塩味は摂りすぎに注意。\n**adviceは足りない味から書き始めないこと。**六味を数えるのは観察であって採点ではありません。まず揃っている味や、その組み合わせの良さを具体的に書く。そのうえで、副菜ひとつで簡単に補える味があるときだけ、最後に一言添える。無理に足す提案をしない（揃っていればそのまま褒めて終わってよい）。
